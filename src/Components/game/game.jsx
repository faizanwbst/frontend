...
class Game extends Component {
  ...
  getDrag(val) {
    if (this.isCurrentPlayerTurn()) {
      this.setState({
        valid_placements: val
          ? this.getValidPlacements(this.state.game.all_dominoes[val])
          : [],
      });
    }
  }
  ...
  isValidPlacement(domino, placement) {
    ...
    if (domino.direction === Left || domino.direction === Right) {
      //if already occupied
      if (
        this.state.game.board[x][y].dot !== Empty ||
        this.state.game.board[x][Math.max(0, y - 1)].dot !== Empty ||
        this.state.game.board[x][Math.min(num_cols - 1, y + 1)].dot !== Empty
      ) {
        return false;
      }
      ...
      if (
        y >= 2 &&
        y < num_cols - 1 &&
        (this.state.game.board[x][y - 2].direction === Left ||
          this.state.game.board[x][y - 2].direction === Right) &&
        (this.state.game.board[x][y - 2].dot === dots1 ||
          dots1 === 0 ||
          this.state.game.board[x][y - 2].dot === 0)
      ) {
        return true;
      }
      ...
      if (
        ...
        x >= 1 &&
        x < num_rows - 1 &&
        (this.state.game.board[x][y - 2].direction === Up ||
          this.state.game.board[x][y - 2].direction === Down) &&
        this.state.game.board[x][y - 2].dot === Separator &&
        this.state.game.board[x - 1][y - 2].dot ===
          this.state.game.board[x + 1][y - 2].dot &&
        (this.state.game.board[x + 1][y - 2].dot === 0 ||
          dots1 === 0 ||
          this.state.game.board[x + 1][y - 2].dot === dots1)
      ) {
        return true;
      }
      //to the left normal
      if (
        ...
        (this.state.game.board[x][y + 2].direction === Left ||
          this.state.game.board[x][y + 2].direction === Right) &&
        (this.state.game.board[x][y + 2].dot === dots2 ||
          dots2 === 0 ||
          this.state.game.board[x][y + 2].dot === 0)
      ) {
        return true;
      ...
      if (
        ...
        (this.state.game.board[x + 1][y + 2].dot === 0 ||
          dots2 === 0 ||
          this.state.game.board[x + 1][y + 2].dot === dots2)
      ) {
        return true;
      }
      //above separator
      if (
        x >= 1 &&
        (this.state.game.board[x - 1][y].direction === Up ||
          this.state.game.board[x - 1][y].direction === Down) &&
        dots1 === dots2 &&
        (this.state.game.board[x - 1][y].dot === dots1 ||
          this.state.game.board[x - 1][y].dot === 0 ||
          dots1 === 0)
      ) {
        return true;
      }
      ...
      if (
        this.state.game.board[x][y].dot !== Empty ||
        this.state.game.board[Math.max(0, x - 1)][y].dot !== Empty ||
        this.state.game.board[Math.min(num_rows - 1, x + 1)][y].dot !== Empty
      ) {
        return false;
      }
      ...
      if (
        x >= 2 &&
        x < num_rows - 1 &&
        (this.state.game.board[x - 2][y].direction === Up ||
          this.state.game.board[x - 2][y].direction === Down) &&
        (this.state.game.board[x - 2][y].dot === dots1 ||
          dots1 === 0 ||
          this.state.game.board[x - 2][y].dot === 0)
      ) {
        return true;
      }
      //below separator
      if (
        ...
        y >= 1 &&
        y < num_cols - 1 &&
        (this.state.game.board[x - 2][y].direction === Left ||
          this.state.game.board[x - 2][y].direction === Right) &&
        this.state.game.board[x - 2][y].dot === Separator &&
        this.state.game.board[x - 2][y - 1].dot ===
          this.state.game.board[x - 2][y + 1].dot &&
        (this.state.game.board[x - 2][y + 1].dot === 0 ||
          dots1 === 0 ||
          this.state.game.board[x - 2][y + 1].dot === dots1)
      ) {
        return true;
      }
      //above normal
      if (
        ...
        (this.state.game.board[x + 2][y].direction === Up ||
          this.state.game.board[x + 2][y].direction === Down) &&
        (this.state.game.board[x + 2][y].dot === dots2 ||
          dots2 === 0 ||
          this.state.game.board[x + 2][y].dot === 0)
      ) {
        return true;
      ...
      if (
        ...
        (this.state.game.board[x + 2][y + 1].dot === 0 ||
          dots2 === 0 ||
          this.state.game.board[x + 2][y + 1].dot === dots2)
      ) {
        return true;
      }
      //to the left separator
      if (
        y >= 1 &&
        (this.state.game.board[x][y - 1].direction === Left ||
          this.state.game.board[x][y - 1].direction === Right) &&
        dots1 === dots2 &&
        (this.state.game.board[x][y - 1].dot === dots1 ||
          this.state.game.board[x][y - 1].dot === 0 ||
          dots1 === 0)
      ) {
        return true;
      }
  ...
  onDrop(ev) {
    ...
    if (ev.target.id && this.isCurrentPlayerTurn()) {
      let game = this.state.game;
      const placement = {
        x: parseInt(ev.target.id.split(",")[0]),
        y: parseInt(ev.target.id.split(",")[1]),
      };
      ...
      game.player_decks[playerIndex] = game.player_decks[playerIndex].filter(
        (k) => {
  ...
  calculateScore(playerDeck) {
    ...
    for (let i = 0; i < playerDeck.length; i++) {
      totalScore +=
        Math.floor(playerDeck[i].dot / 10) + Math.floor(playerDeck[i].dot % 10);
    }
    return totalScore;
  }
  ...
  getEndResult() {
    if (this.isGameInProgress() && deckFilled) {
      ...
      if (
        (placePosition < 0 &&
          this.state.game.player_decks[playerIndex].length === 0) ||
        (placePosition < 0 &&
          game.players > 1 &&
          game.players - game.players_finished.length === 1)
      ) {
      ...
      if (
        this.isEverybodyPlayingStuck() &&
        !game.players_finished.includes(playerIndex)
      ) {
        let sortable = [];
  ...
  getBankDomino() {
    let game = this.state.game;
    const randBankDomino =
      game.bank[Math.floor(Math.random() * game.bank.length)];
  ...
  isGameOver() {
    return (
      this.state.game.players_finished.length === this.state.game.players ||
      this.isEverybodyPlayingStuck()
    );
  }
  ...
  getLastPlayerPlayed() {
    if (this.state.game.players_finished.length > 0) {
      if (playerIndex === this.state.game.players_finished[0]) {
        return (
          this.state.game.players - this.state.game.player_turn - playerIndex
        );
      }
      for (let i = 0; i < this.state.game.registered_users.length; i++) {
  ...
  render() {
    ...
    const missingPlayers =
      this.state.game.players - this.state.game.registered_users.length;
    ...
    const avg =
      statistics.plays_count > 0
        ? Math.floor(statistics.elapsed_time / statistics.plays_count)
        : 0;
    return (
      <div>
        <h1>
          Dominoes <img src={ImageHeadline} alt="dominoesheader" /> Game!
        </h1>
        <table>
          <tbody>
            <tr>
              <td>
                <div className="players">
                  {playerIndex >= 0 && <h2>Player {playerIndex + 1}</h2>}
                  <button disabled={missingPlayers === 0} onClick={this.logout}>
                    Logout
                  </button>
                  <button
                    disabled={
                      missingPlayers === 0 &&
                      !this.state.game.players_finished.includes(playerIndex)
                    }
                    onClick={this.leaveRoom}
                  >
                    Leave Room
                  </button>
                  {this.isCurrentPlayerTurn() && <h2>Your turn</h2>}
                  ...
                  {missingPlayers > 0 && (
                    <h2>
                      Waiting for{" "}
                      {missingPlayers > 1
                        ? missingPlayers + " more players"
                        : missingPlayers + " more player"}
                    </h2>
                  )}
                  {this.state.game.last_move_draw &&
                    !this.isPlayedLastTurn() && (
                      <h4>
                        Player {this.getLastPlayerPlayed() + 1} drew from bank
                      </h4>
                    )}
                  ...
                  {this.state.game.registered_users.map((player) => (
                    <li
                      key={player + "-ingame"}
                      className={player === this.state.username ? "me" : ""}
                    >
                      {player}
                    </li>
                  ))}
              ...
              <td>
                {!this.isGameOver() && (
                  <div>
                    ...
                    >
                      <Board
                        allDominoes={this.state.game.all_dominoes}
                        valid_placements={this.state.valid_placements}
                        dominoes={this.state.game.board}
                      />
                    </div>
                    <h2>Player deck:</h2>
                    <div onDragOver={(e) => Game.onDragOver(e)}>
                      <PlayerDeck
                        allDominoes={this.state.game.all_dominoes}
                        sendDrag={this.getDrag}
                        sendData={this.getData}
                        dominoes={
                          playerIndex < 0
                            ? []
                            : this.state.game.player_decks[playerIndex]
                        }
                      />
                    </div>
                    <div className="control">
                      <button
                        disabled={
                          !currentPlayersTurn ||
                          this.state.game.bank.length === 0
                        }
                        onClick={() => this.getBankDomino()}
                      >
                      ...
                      <button
                        disabled={
                          !currentPlayersTurn ||
                          this.state.game.players === 1 ||
                          !this.isPlayerStuck(playerIndex)
                        }
                ...
                {this.isGameOver() && (
                  <div>
                    ...
                    <table>
                      ...
                      <tbody>{this.createStatisticsTable()}</tbody>
                    </table>
                  </div>
                )}
                <h2>{endResult}</h2>
                  ...
                  <div>
                    ...
                    <ol>
                      {this.state.game.players_finished.map((player) => (
                        <li key={player + "-finished-" + Math.random() * 100}>
                          {this.state.game.registered_users[player]}
                        </li>
                      ))}
                    </ol>
                  </div>
